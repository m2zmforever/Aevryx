local AevryxLib = {RainbowColorValue = 0, HueSelectionPosition = 0}
...
local AevryxGui = Instance.new("ScreenGui")
AevryxGui.Name = "AevryxLib"
...
coroutine.wrap(
	function()
		while wait() do
			AevryxLib.RainbowColorValue = AevryxLib.RainbowColorValue + 1 / 255
			AevryxLib.HueSelectionPosition = AevryxLib.HueSelectionPosition + 1

			if AevryxLib.RainbowColorValue >= 1 then
				AevryxLib.RainbowColorValue = 0
			end

			if AevryxLib.HueSelectionPosition == 80 then
				AevryxLib.HueSelectionPosition = 0
			end
		end
	end
)()
...
function AevryxLib.Main(Name, X, Y)
	CloseBind = AevryxLib.Theme.HideKey or Enum.KeyCode.RightControl
	PresetColor = AevryxLib.Theme.AccentColor
	local fs = false
	...
	function AevryxLib:Notification(HeaderText, Text)
	...
	function Tabs:Tab(text)
        ...
        function TabContent:WelcomeSection(imageId, text)
            local WelcomeFrame = Instance.new("Frame")
            WelcomeFrame.Name = "Welcome"
            WelcomeFrame.Parent = Container
            WelcomeFrame.BackgroundColor3 = Color3.fromRGB(64, 68, 75)
            WelcomeFrame.Size = UDim2.new(0, 457, 0, 60)
            ...
            WelcomeLabel.Position = UDim2.new(0, 60, 0, 0)
            WelcomeLabel.Size = UDim2.new(1, -68, 0, 42)
            WelcomeLabel.Font = Enum.Font.Gotham
            WelcomeLabel.Text = text or ""
            WelcomeLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            WelcomeLabel.TextSize = 16
            WelcomeLabel.TextXAlignment = Enum.TextXAlignment.Left
            WelcomeLabel.TextTransparency = 0.3
            
            Container.CanvasSize = UDim2.new(0, 0, 0, ContainerLayout.AbsoluteContentSize.Y)
            
            local SectionIn = {}
            function SectionIn:SetText(newText)
                WelcomeLabel.Text = newText
            end
            function SectionIn:SetAvatar(newImageId)
                if Avatar then Avatar.Image = newImageId end
            end
            return SectionIn
        end
		...
		function SectionContent:Checkbox(text, callback, defbool)
		...
		function SectionContent:Slider(text, min, max, callback, defvalue)
			local SliderFunc = {}
		...
		function SectionContent:Dropdown(text, list, index, callback)
			local DropFunc = {}
			local Selected = text
			local FrameSize = 43
			local ItemCount = 0
			...
			for k,v in next, list do
				...
				local displayText = (index == 2) and v or k
				local displayVal = (index == 2) and k or v
			...
			ItemCorner.Name = "ItemCorner"
				...
				Item.MouseButton1Click:Connect(function()
					pcall(callback, displayText, displayVal)
					Title.Text = text
		...
		function SectionContent:MultiDropdown(text, list, index, callback)
			local DropFunc = {}
			local Selected = {}
			local DropToggled = false
			local FrameSize = 43
			...
			local function RefreshList()
				...
				for k, v in pairs(list) do
					ItemCount = ItemCount + 1
					if ItemCount == 1 then FrameSize = 78
					elseif ItemCount == 2 then FrameSize = 107
					elseif ItemCount >= 3 then FrameSize = 133 end
					
					local displayKey = (index == 2) and v or k
					local displayVal = (index == 2) and k or v
					...
					Item.MouseButton1Click:Connect(function()
						local idx = table.find(Selected, displayVal)
						if idx then
							table.remove(Selected, idx)
							Item.BackgroundColor3 = Color3.fromRGB(64, 68, 75)
						else
							table.insert(Selected, displayVal)
							Item.BackgroundColor3 = AevryxLib.Theme.AccentColor or Color3.fromRGB(66, 134, 255)
						end
						pcall(callback, displayKey, displayVal)
					end)
				end
			end
			...
			Dropdown.MouseButton1Click:Connect(function()
				...
				if DropToggled then
					Dropdown:TweenSize(UDim2.new(0, 457, 0, FrameSize), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .6, true)
					TweenService:Create(ArrowIco, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = AevryxLib.Theme.AccentColor or Color3.fromRGB(66, 134, 255)}):Play()
				...
				function()
					...
					if RainbowColorPicker then
						...
						while RainbowColorPicker do
							BoxColor.BackgroundColor3 = Color3.fromHSV(AevryxLib.RainbowColorValue, 1, 1)
							Color.BackgroundColor3 = Color3.fromHSV(AevryxLib.RainbowColorValue, 1, 1)

							ColorSelection.Position = UDim2.new(1, 0, 0, 0)
							HueSelection.Position = UDim2.new(0.48, 0, 0, AevryxLib.HueSelectionPosition)
		...
		function SectionContent:TextLabel(text)
		...
		function SectionContent:TextBox(text, callback, defvalue, placeholder)
			if desc == "" then
		...
		function SectionContent:KeyBind(text, callback, defkey)
			local Key = (type(defkey) == "string" and Enum.KeyCode[defkey] or defkey) and (type(defkey) == "string" and Enum.KeyCode[defkey] or defkey).Name or "Unknown"
